...
int main()
{
	...
	try
	{
		...
		while (true)
		{
			...
			cout << "IP:    " << inet_ntoa(c_adr.sin_addr) << endl;
			cout << "Port:  " << htons(c_adr.sin_port) << "\n\n";

			while (true)
			{
				if ((libuf = recv(c, ibuf, sizeof(ibuf), NULL)) == SOCKET_ERROR)
				{
					throw SetErrorMsgText("Recv:", WSAGetLastError());
				}

				if (libuf == 0 || strlen(ibuf) == 0) // условие окончания
				{
					break;
				}
				...
				if (send(c, ibuf, libuf, NULL) == SOCKET_ERROR)
				{
