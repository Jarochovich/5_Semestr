...
bool GetRequestFromClient(char* name, short port, sockaddr* from, int* flen)
{
	...
	{
		throw  SetErrorMsgText("Такой сервер уже есть:", WSAGetLastError());
	}
	...
	while (strcmp(name, message))
	{
		...
		buf = recvfrom(serverSocket, message, strlen(message), NULL, (sockaddr*)&client, &size);
		char* addr = (char*)&client.sin_addr;

		hostent* cl = gethostbyaddr(addr, 4, AF_INET);
		cout << "\n\nИмя хоста клиента: " << cl->h_name << endl;
...
int main() 
{
	...
	try {
		...
		if (gethostname(severName, sizeof(severName)) == SOCKET_ERROR)
			throw SetErrorMsgText("gethostname:", WSAGetLastError());

		cout << "Имя хоста сервера: " << severName;
		...
		while (true) 
		{
			...
			{
				cout << "Есть ответ от клиента: " << PutAnswerToClient(name, (sockaddr*)&clnt, &lc) << " (1 - true / 2 - false)";
			}
			else
			{
				cout << endl << PutAnswerToClient("Error name", (sockaddr*)&clnt, &lc);
			}
