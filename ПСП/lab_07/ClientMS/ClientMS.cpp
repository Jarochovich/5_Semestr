...
int main()
{
    ...
    try
    {
        ...
        {
            cout << SetMailError("Ошибка подключения: ", GetLastError());
            return 1;
        }
        ...
        {
            char wbuf[500] = "Hello from Mailslot-client ";
            ...
            {
                cout << SetMailError("Ошибка отправки: ", GetLastError());
                CloseHandle(mailClient);
                return 1;
            }
            ...
            if (i == 1000)
            {
                ...
                if (!WriteFile(mailClient, wbuf, strlen(wbuf) + 1, &wb, NULL))
                {
                    cout << SetMailError("Error: ", GetLastError());
                    CloseHandle(mailClient);
                    return 1;
                }
            }
        ...
        cout << "Сообщения серверу отправлены" << endl;

        CloseHandle(mailClient);
    }
    catch (...)
    {
        cout << "Необработанное исключение\n";
        return 1;
    }
