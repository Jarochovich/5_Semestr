...
bool GetServer(char* call, short port, sockaddr* from, int* flen)
{
	...
	if ((sendto(cS, call, strlen(call) + 1, NULL, from, *flen) == SOCKET_ERROR))
	{
		throw SetErrorMsgText("sendto:", WSAGetLastError());
	}

	if ((recvfrom(cS, call, sizeof(call), NULL, from, flen) == SOCKET_ERROR))
	{
		return false;
	}
	cout << call << endl;
...
void main()
{
	try
	{
		...
		while (true)
		{
			...
			cout << endl << "IP: " << inet_ntoa(from.sin_addr) << "\tport: " << htons(from.sin_port) << endl;
			lbfrom = sizeof(from);
