...
namespace ht
{
		...
		const wchar_t* fileName)
	{
		...
		std::wstring mutexName = std::wstring(fileName) + L"_MUTEX";
		wprintf(mutexName.c_str());
		HANDLE mutex = CreateMutex(
			NULL,
			FALSE,
			mutexName.c_str());
		htHandle->mutex = mutex;

		std::wstring ownerEventName = std::wstring(fileName) + L"_OWNER";
		htHandle->closeEvent = CreateEvent(NULL, TRUE, TRUE, ownerEventName.c_str());
	...
	{
		std::wstring ownerEventName = std::wstring(fileName) + L"_OWNER";
		HANDLE ownerEvent = OpenEvent(SYNCHRONIZE, FALSE, ownerEventName.c_str());
		if (!ownerEvent) {
			wprintf(L"[ERROR] Owner event not found for: %s (OS11_START probably not running)\n", ownerEventName.c_str());
			return NULL;
		}
		...
		const HtHandle* htHandle)           // управление HT (File, FileMapping)
	{
		...
		// закрываем событие владельца
		std::wstring ownerEventName = std::wstring(htHandle->fileName) + L"_OWNER";
		HANDLE ownerEvent = OpenEvent(SYNCHRONIZE, FALSE, ownerEventName.c_str());
		if (ownerEvent) {
			CloseHandle(ownerEvent);
		}
		...
		ht::snap(const_cast<HtHandle*>(htHandle));
